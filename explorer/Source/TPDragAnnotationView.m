...
@implementation TPDragAnnotationView

- (id)initWithAnnotation:(id <MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier {
	self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier];
    
	if (self) {
		self.image = [UIImage imageNamed:@"drag_icon"];
        [self clearTouch];
	}
	return self;
}
...
- (void)updateCoordinateFromTouch:(UITouch *)touch
{
    ...
    CLLocationCoordinate2D newCoordinate = [self.mapView convertPoint:point toCoordinateFromView:nil]; // nil means "use the application window"
